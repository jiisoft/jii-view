...
Jii.defineClass('Jii.view.react.widgets.BaseListView', /** @lends Jii.view.react.widgets.BaseListView.prototype */{
    ...
    render() {
        ...
        if (this.props.showOnEmpty || this.props.collection.length > 0) {
            content = Jii._.map(this.props.layout.match(/{\w+}/g), this.renderSection.bind(this));
        } else {
            content = [this.renderEmpty()];
        }

        var options = Jii._.clone(this.props.options);
     ...
     */
    renderEmpty() {
        var options = Jii._.clone(this.props.emptyTextOptions);
        var tag = options.tag || 'div';
        delete options.tag;
