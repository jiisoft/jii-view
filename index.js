module.exports = require('jii');

require('node-jsx').install({extension: '.jsx'});

// AUTO-GENERATED Package files list
require('./lib/IRenderer.js');
require('./lib/View.js');
require('./lib/ViewEvent.js');
require('./lib/WebView.js');
require('./lib/react/LayoutView.js');
require('./lib/react/ReactRenderer.js');
require('./lib/react/ReactView.js');
require('./lib/react/form/ActiveField.jsx');
require('./lib/react/form/ActiveForm.jsx');
require('./lib/react/form/Button.jsx');
require('./lib/react/form/CheckBox.jsx');
require('./lib/react/form/CheckBoxList.jsx');
require('./lib/react/form/DropDownList.jsx');
require('./lib/react/form/Input.jsx');
require('./lib/react/form/RadioList.jsx');
require('./lib/react/form/TextArea.jsx');
require('./lib/react/grid/Column.js');
require('./lib/react/grid/DataColumn.js');
require('./lib/react/grid/GridView.js');
require('./lib/react/server/ReactServerRenderer.js');
require('./lib/react/widgets/BaseListView.js');
require('./lib/server/ServerWebView.js');
require('./lib/server/underscore/UnderscoreRenderer.js');
